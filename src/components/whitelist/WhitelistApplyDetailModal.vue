<template>
  ...
  <q-dialog v-model="whitelistApplyDetailModal" :maximized="maximized" transition-show="slide-up" transition-hide="slide-down">
    ...
    <q-layout view="lHh Lpr lFf" container class="shadow-2 rounded-borders bg-white" style="min-width: 40%;">
      ...
      <q-card style="word-break: break-word;" class="q-pt-md">
        <q-card-section>
          ...
          <div class="row justify-center">
            <div class="col-12">
              <q-input v-model="total_score" ref="title" type="number" clearable standout tabindex="1" />
            </div>
          </div>
          ...
          <div class="row justify-center q-pt-lg q-pb-xl">
            ...
            <div class="col-6 text-right">
              <q-btn class="btn" color="secondary" text-color="black" size="lg" style="width: 98%;" @click="updateWhitelistApplyStatusCd" tabindex="4">
                <b>{{ $t('modify') }}</b>
              </q-btn>
            </div>
          </div>
...
export default {
  name: 'WhitelistApplyDetailModal',
  data () {
    return {
      whitelistApplyDetailModal: false,
      maximized: false,
      select_type: '',
      seq: '',
      seq_whitelist: '',
  ...
  methods: {
    async show () {
      ...
      this.whitelistApplyDetailModal = true

      // 화이트리스트 신청정보 조회
      await this.selectWhitelistApplyBySeq()
    ...
    // 화이트리스트 신청정보 조회
    async selectWhitelistApplyBySeq() {
      const param = {
        uid: this.getUid,
        seq: this.seq,
      }
      const result = await this.$axios.get('/api/whitelist/selectWhitelistApplyBySeq', { params: { ...param } })
      if (result.data) {
    ...
    updateWhitelistApplyStatusCd() {
      ...
      this.$axios.post('/api/whitelist/updateWhitelistApplyStatusCd', param)
        .then((result) => {
    ...
    close () {
      this.whitelistApplyDetailModal = false
    }
  }
}
</script>
